/* Program to Create Table_04: Effects on Administrative Enrollment in Following Year */
...
set more off;
capture log close;
log using Table_04_Enrollment.log, replace text;
...
outreg T1_treat T2_treat using "Table_04_Enrollment.csv", 3aster coefastr se comma replace;
test T1_treat == T2_treat;

xi: reg m_enrolled T1_treat T2_treat $varbaseline if suba == 0 & grade < 11, cluster(school_code);
outreg T1_treat T2_treat using "Table_04_Enrollment.csv", 3aster coefastr se comma append;
test T1_treat == T2_treat;
...
outreg T3_treat using "Table_04_Enrollment.csv", 3aster coefastr se comma append;
...
outreg T1_treat T2_treat T3_treat using "Table_04_Enrollment.csv", 3aster coefastr se comma append;
test T1_treat == T2_treat;
test T1_treat == T3_treat;

*Control Mean;
su m_enrolled if treat ~= 1 & grade > 8 & grade < 11;

*Check Balance of missing observations;
gen m_enrolled_miss = m_enrolled == .;
su m_enrolled_miss if suba == 0;
reg m_enrolled_miss T1_treat T2_treat if suba == 0, cluster(school_code);
su m_enrolled_miss if suba == 1;
reg m_enrolled_miss T3_treat if suba == 1, cluster(school_code);
