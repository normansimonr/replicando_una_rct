/* Program to Create Appendix A1: Comparison of Students in Schools Selected for Attendance Monitoring and Surveying */
...
set more off;
capture log close;
log using Table_A1_Survey_Selected_Comparison.log, replace text;
...
foreach var in $varbaseline {;
   ...
   reg `var' treatment if suba == 1, cluster(school_code);
   sig col7 _b[treatment] _se[treatment] row_num `row';

   };

sort row_num;
outsheet col0 col1-col8 using Table_A1_Survey_Selected_Comparison.csv if _n <= `row'+1, replace comma;
log close;
