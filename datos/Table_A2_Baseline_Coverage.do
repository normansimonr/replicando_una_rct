/* Program to Create Appendix A2: Relative Differences between Students Covered and not Covered in Baseline */
...
set more off;
capture log close;
log using Table_A2_Baseline_Coverage.log, replace text;
...
foreach var in $varbaseline {;
   ...
   reg `var' fu_attrit if suba == 0 & control == 1;
...
sort row_num;
outsheet col0 col1-col8 using "Table_A2_Baseline_Coverage.csv" if _n <= $row+1, replace comma;
log close;
