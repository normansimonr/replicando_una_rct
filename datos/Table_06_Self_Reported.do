/* Program to Create Table_06: Effects on Self-Reported Attendance and Enrollment Rates, Grades 6-10 */
...
set more off;
capture log close;
log using Table_06_Self_Reported.log, replace text;
...
outreg T1_treat T2_treat using "Table_06_Self_Reported.csv", 3aster coefastr se comma replace;
test T1_treat == T2_treat;

su fu_self_attendance if treatment ~= 1 & suba == 0;

xi: reg fu_self_attendance T3_treat $varbaseline i.school_code if suba == 1, cluster(school_code);
outreg T3_treat using "Table_06_Self_Reported.csv", 3aster coefastr se comma append;

su fu_self_attendance if treatment ~= 1 & suba == 1;

xi: reg fu_self_attendance T1_treat T2_treat T3_treat $varbaseline i.school_code if grade > 8, cluster(school_code);
outreg T1_treat T2_treat T3_treat using "Table_06_Self_Reported.csv", 3aster coefastr se comma append;
test T1_treat == T2_treat;
test T1_treat == T3_treat;

su fu_self_attendance if treatment ~= 1 & grade < 8;

xi: reg fu_currently_attending T1_treat T2_treat $varbaseline i.school_code if suba == 0, cluster(school_code);
outreg T1_treat T2_treat using "Table_06_Self_Reported.csv", 3aster coefastr se comma append;
test T1_treat == T2_treat;

su fu_currently_attending if treatment ~= 1 & suba == 0;

xi: reg fu_currently_attending T3_treat $varbaseline i.school_code if suba == 1, cluster(school_code);
outreg T3_treat using "Table_06_Self_Reported.csv", 3aster coefastr se comma append;

su fu_currently_attending if treatment ~= 1 & suba == 1;
...
su fu_currently_attending if treatment ~= 1 & grade > 8;
